...
`lib/src/reporting/channels.ts` owns the records and matching rules. The standalone [reporting reference](../reporting-channels.md) is generated from those records. The email analyzer derives qualified provider-role candidates, calls this lookup, and supplies relevant results to optional Flue assessment.
...
The lookup scans the six records, matching provider and role together. The former Flue batch binding is retired; direct library calls and the batch input schema remain available. No search index or network discovery is needed.
...
The analyzer may select channels for an evidence-backed plausible recipient while involvement remains unconfirmed. The assessment must retain that distinction and state the evidence. The query and returned references do not encode attribution confidence. This guidance makes a lead eligible for investigation; it does not satisfy the returned channel conditions or authorize sending. The analyzer owns the limited [reporting derivations](email-analysis.md#reporting-derivations); the channel lookup does not infer roles.
