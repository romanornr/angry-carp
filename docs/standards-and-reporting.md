...
| Contact discovery and catalogue design | [Reporting-channel research](research/reporting-channel-catalogue.md) | [RFC 2142](https://www.rfc-editor.org/rfc/rfc2142.html), [RFC 9116](https://www.rfc-editor.org/rfc/rfc9116.html), [ICANN RDAP Response Profile](https://www.icann.org/en/system/files/files/rdap-response-profile-21feb24-en.pdf) |
...
ARF and IODEF exports are not implemented. The [analyzer's RDAP lookups](email-analysis.md#rdap-discovery-and-failures) use RFC 9224 service discovery, RFC 9082 domain queries, and selected RFC 9083 response fields. They support a bounded subset, with no redirects, referrals, or WHOIS fallback. Registrar abuse contacts retain their entity relationship, consistent with the [ICANN gTLD RDAP profile](https://www.icann.org/gtld-rdap-profile). An operator-supplied lookup remains supplied evidence, and reading standards does not count as checking a case's domains.

The separate [IP lookup](../lib/README.md#use-a-module) uses RFC 9224 section 5's longest binary prefix, RFC 9082 section 3.1.1's address query and RFC 9083 section 5.4's network object. It validates the returned family and enclosing range. These are selected protocol behaviors, not a claim of full RDAP conformance: both lookups omit redirects and endpoint failover. The analyzer reuses fresh discovery data within one run under a conservative HTTP cache policy. Registered network, authoritative DNS and origin hosting are distinct relationships.
...
The [channel reference](../reporting-channels.md) is generated from the typed catalogue, with conditions and per-record review dates. Flue retrieves relevant records through an offline lookup, as decided in [ADR 0009](adr/0009-maintain-reporting-channels-as-data.md). Cloudflare's form is included; its entitlement-limited API remains research only. Provider attribution, report readiness, and sending approval remain separate. Submission automation is not implemented.
