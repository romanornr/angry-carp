...
The separate [IP lookup](../agent/README.md#ip-registration-lookups) uses RFC 9224 section 5's longest binary prefix, RFC 9082 section 3.1.1's address query and RFC 9083 section 5.4's network object. It validates the returned family and enclosing range. These are selected protocol behaviors, not a claim of full RDAP conformance: both lookups omit redirects, endpoint failover and bootstrap caching. Registered network, authoritative DNS and origin hosting are distinct relationships.
