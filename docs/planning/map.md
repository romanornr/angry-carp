...
- The operator accepts browser authorization to Composio and Google consent for Gmail access. Avoid requiring users to create a Google Cloud OAuth application. Composio remains a candidate; this preference does not authorize connecting an account. The operator subsequently authorized the Flue starter in `agent/` and dependency installation. Authentication and Gmail tools remain unconnected.
- The operator proposed reusing the existing Codex login. The [integration investigation](../research/codex-login-reuse.md) confirms Codex-managed authentication as a supported direction, but not automatic reuse by Flue's direct provider. Replacing Flue or building a custom provider remains undecided. Do not create another OpenAI token store or silently fall back to API billing.
- The subsequent synthetic check passed using the installed Codex CLI directly because SDK 0.155.1 does not expose the configuration-isolation and ephemeral-session flags. `agent/` documents `npm run check:codex`. Codex returned `Medium` and `reportReady: false` using the existing ChatGPT login. This is a bounded login and response check, not authorization to process real mail or a final runner decision.
...
- The operator selected local execution initially. Cloudflare may be considered later. A local Flue analyst runner remains under discussion; local execution does not require using an existing coding agent as the analyst.
