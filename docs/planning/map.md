...
[Issue 21](issues/21-handle-rdap-discovery-unavailability.md) records reproduced IANA discovery name-resolution failures, a later full-run failure of unknown stage, and a successful isolated follow-up. It covers bounded diagnostics and discovery reuse guided by RFC 9224. [Issue 22](issues/22-preserve-domain-relationship-uncertainty.md) tracks the residual AI claim that a different sender domain is unrelated.
...
Report-preparation research remains deferred under issue 18. Unsupported HTML, fresh authentication verification and payload examination remain explicit coverage limits, not newly authorized expansion. Resolve RDAP reliability first; address the smaller wording gap without adding another model pass.
