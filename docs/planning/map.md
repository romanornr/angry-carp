...
- The first local Flue integration assessed prepared text and selected reporting channels through a tool. The current analyzer replaces that orchestration. [ADR 0014](../adr/0014-keep-provider-routing-outside-ai-assessment.md) narrows Flue to evidence assessment using `agent/phishing-assessment.md`; the runtime displays provider roles and routes. The model has no filesystem, shell, browser or mailbox tools.
...
[ADR 0013](../adr/0013-route-assessment-by-concerns-and-coverage.md) records automatic AI routing and bounded recovery. [Issue 18](issues/18-verify-evidence-during-report-preparation.md) now implements host-assisted preparation: an existing AI host researches and drafts, and the standalone command checks attributed records and exact input bindings. [ADR 0015](../adr/0015-attribute-host-report-research.md) records the unverified host boundary. Native research, durable approval and sending remain future work. Registration dates remain in the in-memory result and model projection; routine JSON export stays optional.

The subsequent live retest retained High concern and included the image/action mismatch and qualified Resend lead. It completed with no model-directed tool calls. [Issue 19](issues/19-preserve-evidence-limits-in-ai-assessments.md) records the later correction for AI role over-inference: remove attribution records from its input and render those records deterministically. The scoped replay retained the deception reasoning without reconstructing providers or recipients. Private comparisons remain under ignored `evidence/emails/`. These case runs do not establish detection accuracy across other mail.

The [assessment-limits investigation](../research/assessment-evidence-limits.md) distinguishes model over-inference from a separate, reproduced lookup-classification defect. [Issue 20](issues/20-classify-malformed-lookup-json.md) records the implemented malformed-JSON correction. It does not establish what caused the earlier live RDAP failure.

[Issue 21](issues/21-handle-rdap-discovery-unavailability.md) now implements bounded failure diagnostics and run-owned discovery reuse guided by RFC 9224. A live two-domain check succeeded with one discovery request and two registry requests; the historical failed run's cause remains unknown. [Issue 22](issues/22-preserve-domain-relationship-uncertainty.md) revises the domain-mismatch clause in both instruction documents. Its paired synthetic evaluation retains the unsupported "unrelated" baseline claim and the revised output; these samples do not establish general model accuracy.

The full-pipeline retest at 2026-09-22T22:24Z completed in 31.453 seconds, retained the image/action explanation and displayed provider candidates deterministically. The AI did not reconstruct proxy/hosting roles or recipients. One domain lookup still failed. This verifies the normal command after the narrower replay, not general accuracy or a controlled performance improvement.

Report-preparation research now uses the host-assisted workflow under issue 18; native execution remains deferred. Unsupported HTML, fresh authentication verification and payload examination remain explicit coverage limits, not newly authorized expansion. Issue 22's instruction-only correction failed full-pipeline retests. [ADR 0016](../adr/0016-render-recorded-assessment-evidence.md) now replaces local free-form evidence prose with a structured conclusion and selections rendered from recorded facts. Offline gates, synthetic controls and real-email retests passed; Claude closed the independent review. This adds no assessment model pass.
