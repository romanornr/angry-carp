...
Following the operator's concern about the value and privacy cost of Netcraft forwarding, the replacement excludes it from the default reporting path. An operator may still explicitly request it for a case, subject to the same payload and disclosure review. Reporting to providers does not require a Netcraft submission or verdict.
